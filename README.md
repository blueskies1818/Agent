...
├── start.sh                     # Unified launcher — venv, deps, Docker build + start
├── wipe.py                      # Wipe memory, logs, vectors, or workspace
├── requirements.txt
├── dockerfile                   # Docker sandbox image (Ubuntu + tools + Firefox)
├── docker-compose.yml           # Container config (optional — start.sh handles this)
├── .dockerignore
...
│   ├── engine/     ✗ unreachable   x  │   ├── project files
│   ├── config.py   ✗ unreachable    x │   └── (AI reads and writes here)
│   ├── .env        ✗ unreachable     x│
│   └── workspace/ ─── bind mount ──→ x│── root access (apt-get, pip, etc.)
...
| Command | What gets mounted |
|---------|-------------------|
| `SANDBOX=docker ./start.sh` | `./workspace/` → `/workspace` |
| `PROJECT=/path ./start.sh` | `/path` → working directory (local mode) |
| `PROJECT=/path SANDBOX=docker ./start.sh` | `/path` → `/workspace` (Docker mode) |
...
```bash
PROVIDER=claude TIER=fast SANDBOX=docker ./start.sh
```
